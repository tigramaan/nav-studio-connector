...
| V-003 Ubuntu disposable CA | PASS | Ephemeral Ubuntu HIL installed connector CA `BD7A9BDD…A0275`, strict HTTPS passed, connector-owned removal completed and `trust_cleanup=true`; host trust and package cleanup succeeded. |
| V-004 post-install HTTPS | PASS | Windows fresh `curl.exe`/Schannel and Ubuntu fresh native/curl probes passed strict TLS after installation; TLS verification was never disabled. |
...
| V-007 Ubuntu package | PASS | Ubuntu 22.04 VM release CLI built and ran. Public package CI installed the real `.deb`, exercised installed CLI, kept the installed GUI alive for five seconds, exercised AppImage CLI and uploaded both artifacts. |
...
- CI run `33165650133`: Windows and Ubuntu jobs completed successfully at commit `1556e2d`.
- Package run `33165650027`: Windows and Ubuntu jobs completed successfully; Ubuntu HIL JSON reported `.deb` install, installed binary, five-second GUI smoke and `trust_cleanup=true`.
...
- Branch Windows artifacts are unsigned development outputs and may trigger SmartScreen. The first production `v*` tag remains blocked until the organization supplies the documented CA-issued signing PFX secrets; the workflow then requires signature and timestamp.
